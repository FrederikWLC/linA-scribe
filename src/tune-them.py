...
from model.baselines.sam import (
...
from utils.metrics import BinaryDiceScore, evaluate_model
...
def perform_tuning(n_trials=100):
    ...
    for model in MODELS:
        ...
        df_trials.to_csv(f"data/tuning-{model.name}-trials.csv", index=False)
        ...
        df_best.to_csv(f"data/tuning-{model.name}.csv", index=False)

...
def auto_brush(thresh,d_prb_erosion=3) -> np.ndarray:
    prb_erosion_kernel = cv2.getStructuringElement(shape=cv2.MORPH_ELLIPSE, ksize=(d_prb_erosion, d_prb_erosion))
    ...
    sure_fgd_brush = cv2.morphologyEx(thresh, cv2.MORPH_ERODE, prb_erosion_kernel)

...
- `POST /api/evaluation/tuning/run`
- `POST /api/evaluation/evaluation/run`
- `POST /api/evaluation/ablation/run`
- `POST /api/evaluation/scribing/run`
- `POST /api/evaluation/autoprompt/run`

## Local Evaluation Runs (Outside Docker)

If you want to run tuning/evaluation/ablation directly on your local machine (not through Docker backend routes), use:

```powershell
python run_local_eval.py tune --n-trials 100
python run_local_eval.py evaluate
python run_local_eval.py ablate
python run_local_eval.py all --n-trials 50
```

This runner executes from `src/` so existing relative `data/...` outputs continue to write into `src/data`.

Evaluation flow:

- Lists existing files from Supabase (signed URLs), split by process.
- Backend mounts the local `src` folder at `/workspace/src`.
...
- Backend (`FastAPI`): edits in `backend/app` trigger automatic reload.

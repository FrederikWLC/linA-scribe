...
def _prepare_local_python_context() -> None:
    ...
    # Ensure imports like src.* resolve from local host runs.
...
def run_local(task: str, n_trials: int) -> None:
    ...
    if task in {"tune", "all"}:
        from src.tune_them import perform_tuning

        perform_tuning(n_trials=n_trials)

    if task in {"evaluate", "all"}:
        from src.evaluate_them import run_default_evaluation

        run_default_evaluation()

    if task in {"ablate", "all"}:
        from src.ablate_SAM import run_ablation

        run_ablation()

    if task in {"compare", "all"}:
        from src.compare_them import run_default_comparison

        run_default_comparison()

    if task in {"autoprompt", "all"}:
        from src.save_autoprompt_displays import run_autoprompt_export

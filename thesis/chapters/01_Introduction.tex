...
The aim of this project was therefore to investigate whether parts of this process can be automated.
More specifically, whether some kind of semi-automatic segmentation tool can accelerate the workflow, while maintaining high-quality outputs that are still compatible with SigLA's database.
...
In particular Meta's state-of-the-art interactive segmentation model, the \acf{SAM} \parencite{sam}, which is pre-trained on a massive and diverse dataset - \acf{SA-1B}, of over 1 billion masks.
...
However, fortunately, it has equally been shown that \acs{SAM}'s performance can significantly improve through effective prompt engineering alone \parencite{promptSAM}, requiring no deep learning at all.
...
More specifically, whether some kind of semi-automatic segmentation tool using interactive and generalized \acs{CV} - specifically the \acf{SAM} - can accelerate the workflow, while maintaining high-quality outputs that are still compatible with SigLA’s database.
...
To concretize the research, the problem statement was therefore decomposed into three \acfp{RQ} - each accompanied by their respective operationalization (see the following subsection for details).

\subsection{Research questions}
\begin{enumerate}
\item \textbf{\acs{RQ}1 (Segmentation performance)}: 
...
\textbf{Operationalization}: \acs{SAM}-based masks will be compared to classical image-processing baselines using overlap metrics (e.g. \acs{IoU}/\acs{Dice}) against the ground truth segmentations derived from SigLA's Krita files for a minimum of 25 documents.
\item \textbf{\acs{RQ}2 (Degree of automation)}: 
...
\textbf{Operationalization}: The number of user interactions (e.g. clicks) required to achieve a certain level of segmentation quality (e.g. \acs{IoU}/\acs{Dice}) will be measured against ground truth (SigLA's Krita files) for a minimum of 25 documents.
...
\textbf{Operationalization}: The time it takes Ester Salgarella to annotate manually compared to being assisted by the proposed segmentation tool will be measured for a minimum of 5 documents.
\end{enumerate}

\section{Impact -- innovation and application}

The work of this thesis presents itself as a first proof-of-concept (and a mapping of the difficulties involved) in using \acs{SAM} and modern \acs{CV} to accelerate the expansion of the SigLA database.
Its main contribution is a prototype of a semi-automatic annotation tool, along with an evaluation framework that can be used to assess the performance of itself and any potential future versions.
A future version of this tool could potentially also be applied to Linear B, as it is the whole field of Aegean studies that seems to suffer from severe under-digitization of the primary evidence (which is a huge problem for the scientific investigations of scholars).

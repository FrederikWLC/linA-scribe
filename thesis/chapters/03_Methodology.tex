...
To determine whether one model performed significantly better than another, relevant metrics and statistical tests needed to be defined.
Furthermore, to ensure a fair comparison, all models with hyperparameters were had to be optimized to their best performance on the validation set.
Naturally, as a last check of \acs{SAM}'s efficiency, it was of particular interest to statistically compare its best performing version against the best performing classical baseline.
...
It is naturally more foregiving than the related segmentation metric \acs{IoU}, which is more strict and specifically used to evaluate segmentation results where small displacements are critical for results.
...
\noindent Thus, the raw metrics ended up consisting of the \acs{Dice} scores for each image, and each model.

\subsubsection{Hyperparameter optimization}
\label{subsubsec:hyperparameter_optimization}
To ensure a fair comparison between the models, all models with hyperparameters were tuned to their best performance on the validation set in a series of 100 trials each using Optuna, a hyperparameter optimization framework.
...
A \acs{TPE} works by building two probabilistic models: one for the configurations that have resulted in good performance, and another for those that have resulted in poor performance.
...
In case the normality assumption were to be violated, the non-parametric domain-specific Wilcoxon signed-rank test would be used instead \parencite{Rainio2024}.

...
These support images $x_s^{i}$ are chosen by a ranking of feature similarity to the input $x^i$ to make sure that the support masks $y_s^{i}$ are as representative as possible.
This ranking is computed ascendingly from the Manhattan distances between the image-encoded features of the input and the support images.

\begin{figure}[htp]
\centering
    \includegraphics[width=\linewidth]{images/FATESAM-architecture.jpg}
\caption{Architecture of \acs{FATESAM}, from \textcite{fatesam}.
At inference of the 2D-adaptation, support masks $y_s^{i}$ are chosen by the similarity of their corresponding image-encoded support images $\mathcal{E}(x_s^{i})$ to the image-encoded query image, $\mathcal{E}(x^i)$ and passed to memory attention $\mathcal{MA}$ as memory encoded embeddings $\mathcal{ME}(y_s^{i})$.
However, unlike the original 3D setting, no adjacent
prediction $\mathcal{ME}(y^{i-1})$ is relevant, since the Linear A images are
treated as independent two-dimensional inputs.}

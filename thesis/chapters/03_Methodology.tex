...
The choice of this programming language was made primarily due to it being the language of choice for the \acs{ML} (e.g. OpenCV, Optuna, etc.) and \acs{SAM} implementation libraries needed.
Its big community, and library support, and package manager (pip) also made it exceptionally easy to install and manage dependencies, when setting up the development environment.
The particular version was chosen, as it was the second latest stable release at the time of development \parencite{python313release}, with the risk of Python 3.14 still not having full support by all libraries.

\section{Non-interactive segmentation} \label{Segmentation quality}
To answer the question of \hyperref[rq1]{\acs{RQ}1}: \textit{How well does a \acs{SAM}-based segmentation approach perform quantitatively in terms of segmentation accuracy, compared to simple classical image processing baselines when applied to Linear A document images?}, the initial fully-automatic segmentation quality of \acs{SAM} was evaluated against a set of classical baseline models, using a suitable segmentation metric (i.e. \acs{Dice} score) computed from the pairs of predictions and ground truth masks.
...
The following subsections give an explanation of the segmentation problem itself, and document the configuration of the baseline and state-of-the-art models, as well as the evaluation protocol used to answer \hyperref[rq1]{\acs{RQ}1}.
...
In this way, more noise could be reduced while preserving engravings, as this filter is known to be effective at blurring out noise while preserving edges, thanks to its combined distance and intensity-dependent weighting \parencite{TomasiManduchi1998}.
...
Otsu's method then works by performing an exhaustive search over all $L$ gray levels to find an optimal threshold $T^{*}$ that results in a maximum between-class variance $\sigma^2_B$ \parencite{Otsu1979}:
...
With a Gaussian kernel  $G_{\sigma}$, the threshold $T(x,y)$ is computed as a Gaussian-weighted sum of local intensities subtracted by a constant $C$. Using kernel convolution ($*$), this can be expressed as \parencite[see][]{opencv_adaptive_threshold}\parencite[p.~761]{DigitalImageProcessing}:
...
Canny edge detection \parencite{Canny1986} is a method that by itself only segments the edges of an image, excluding the enclosed regions within those edges.
...
\noindent Then, in an attempt to fill the regions enclosed by the detected edges, morphological closing (dilation followed by erosion) was applied to the edge mask \parencite[p.~636--638]{DigitalImageProcessing}.
...
To be compared against the classical baselines was the foundational \acf{SAM} \parencite{sam} pre-trained on a huge dataset of natural images \acs{SA-1B}.
...
This image encoder takes the form of a \acf{ViT}, that similarly to a \acf{CNN} \parencite{LeCun1998} processes an input image in order to extract its features \parencite{imageworth16x16words}.
Instead of relying on just convolutional layers, however, a \acs{ViT} adapts the transformer architecture originally developed for \acs{NLP} \parencite{vaswani2023attentionneed} to the image domain.
...
Mobile\acs{SAM}, by definition, instead relied on the lighter \acs{ViT} variant TinyViT \parencite{TinyViT}, whose 5M parameters are far fewer than those of the \acs{SAM} encoders (ViT-H, ViT-L, and ViT-B, with 632M, 307M, and 86M parameters, respectively) \parencite{sam}.
...
\acs{SAM}2 also takes a checkpoint-specific YAML configuration file, which stores the hyperparameters for the model, and allows for easy overrides through Hydra \parencite{HydraFramework}.
...
The memory and video-tracking machinery is only relevant to the \acs{FATESAM} adaptation described in \cref{subsubsec:FATESAM}.
...
As an adaptation to the data domain, the one-shot support image was selected in the same way as for \acs{FATESAM} (see \cref{subsubsec:FATESAM}): the support images were ranked by the Manhattan distance of their DINOv2 features to those of the query image, and the most similar one was selected as support.
...
Without the hyperparameters of \acs{Gaussian}, this method thus introduces three hyperparameters: $n_{fgd}$, $n_{bgd}$, and the erosion kernel size $d$ \parencite[p.~636--637,639]{DigitalImageProcessing}.
...
\subsection{Evaluation protocol for \hyperref[rq1]{\acs{RQ}1}}
...
It is naturally more forgiving than the related segmentation metric \acs{IoU}, which is more strict and specifically used to evaluate segmentation results where small displacements are critical for results \parencite{Rainio2024}.
...
This included only all classical models, as well as the \acs{SAM} variants with automatic prompts and bilateral pre-filtering.
...
Specifically, a one-tailed paired t-test was used, as the alternative hypothesis was that \acs{SAM} would perform significantly worse than the best classical baseline, instead of just significantly differently.
The paired t-test has two main assumptions: 1. independence of tested observations, and 2. normality of the paired differences \parencite{Rainio2024}.
...
Thus, the paired differences in \acs{Dice} scores between the two models tested needed to be checked for normality using a QQ-plot and the Shapiro-Wilk test \parencite{ShapiroWilk1965}.
In case the normality assumption was violated, the non-parametric Wilcoxon signed-rank test would be used instead \parencite{Rainio2024}.
...
To make the development environment more consistent with production, the application was containerized using Docker \parencite{DockerDocs}, which made it easier to deploy and run on any machine without worrying about dependencies and environment setup.

Svelte was chosen for the frontend mostly due to its reactive state management, making it well-suited for building interactive user interfaces, as this functionality removes the need for re-inserting state variables every time they change \parencite{SvelteDocs}. 
...
It is also lightweight and asynchronous, which potentially allowed for many users to use the tool simultaneously, as several requests could be handled at the same time without blocking execution on the server \parencite{FastAPIDocs}.
...
This was deemed appropriate because the non-interactive evaluation described in \cref{Segmentation quality} could not determine how \acs{SAM} would perform when used interactively.
...
\subsection{Experimental design for \hyperref[rq2]{\acs{RQ}2}}
\label{subsec:experimental_design_interaction_efficiency}
To evaluate the interaction efficiency of the \acs{SAM}-based tool, a case study was conducted with Ester Salgarella, where she was asked to perform the segmentation of 3 documents from the test set (one from each difficulty category, see \cref{sec:dataset}) with each method of the tool.
...
When Ester does the annotation of a document from the \acs{GORILA} corpus \parencite{EtudesCretoises21}, she first imports a screenshot of the \acs{GORILA} corpus drawing into Krita \parencite{KritaDocs}, creating a new layer on top of it to do the segmentation drawing.
...
At last, when she is satisfied with the drawing, she splits each sign into a separate layer, adds metadata, and imports to SigLA \parencite{SalgarellaCastellanSigLA2020} as JSON and Krita files.
...
The latter is an open file format that like Krita (\texttt{.kra}) files consists of a ZIP file with an XML document describing the layer structure, together with a PNG or SVG image for each raster or vector layer, respectively \parencite{OpenRasterSpec}. 
...
Krita files, on the other hand, are more complex in structure, including proprietary elements, and not designed for external programmatic access, as they are meant to be used within Krita only \parencite{KritaDocs}. 
...
\subsection{Experimental design for \hyperref[rq3]{\acs{RQ}3}} \label{subsec:experimental_design_workflow_efficiency}

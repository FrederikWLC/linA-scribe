...
predictions $\mathcal{ME}(y^{i-1})$ are relevant, since a Linear A image is
...
As seen in \cref{fig:GFSAM_architecture}, it works by first extracting the features of the given support image $x^r$ and the query image $x^t$ by using the pre-trained image encoder DINOv2 \parencite{dinov2}.
As an adaptation to the data domain, the selection of this support image for \acs{GFSAM} was done in the same way as for \acs{FATESAM}, by ranking the feature similarity of the support images to the input image via Manhattan distance on the extracted features, and selecting the most similar one as one-shot support.
...
However, if no points remain after the gating processes, the mask with the highest predicted IoU score is selected as the final mask.
\begin{figure}[htp]
\centering

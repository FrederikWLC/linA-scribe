...
The particular version was chosen, as it was the second latest stable release at the time of development \parencite{python313release}, with the risk of Python 3.14 still not having full support by all libraries. % NEW: python313release -- Python release schedule / 3.13 release announcement (python.org)
...
Canny edge detection \parencite{Canny1986} is a method that by itself only segments the edges of an image, excluding the enclosed regions within those edges. % NEW: Canny1986 -- J. Canny (1986), A Computational Approach to Edge Detection, IEEE TPAMI 8(6)
...
\noindent Then, in an attempt to fill the regions enclosed by the detected edges, morphological closing (dilation followed by erosion) was applied to the edge mask \parencite{DigitalImageProcessing}. % TODO: add page ref for the morphological operations chapter
...
Thus, the paired differences in \acs{Dice} scores between the two models tested needed to be checked for normality using a QQ-plot and the Shapiro-Wilk test \parencite{ShapiroWilk1965}. % NEW: ShapiroWilk1965 -- S. S. Shapiro & M. B. Wilk (1965), An analysis of variance test for normality, Biometrika 52(3-4)
...
To make the development environment more consistent with production, the application was containerized using Docker \parencite{DockerDocs}, which made it easier to deploy and run on any machine without worrying about dependencies and environment setup. % NEW (optional): DockerDocs -- Docker official documentation

Svelte was chosen for the frontend mostly due to its reactive state management, making it well-suited for building interactive user interfaces, as this functionality removes the need for re-inserting state variables every time they change \parencite{SvelteDocs}. % NEW (optional): SvelteDocs -- Svelte official documentation
...
It is also lightweight and asynchronous, which potentially allowed for many users to use the tool simultaneously, as several requests could be handled at the same time without blocking execution on the server \parencite{FastAPIDocs}. % NEW (optional): FastAPIDocs -- FastAPI official documentation
...
The latter is an open file format that like Krita (\texttt{.kra}) files consists of a ZIP file with an XML document describing the layer structure, together with a PNG or SVG image for each raster or vector layer, respectively \parencite{OpenRasterSpec}. % NEW: OpenRasterSpec -- OpenRaster specification (freedesktop.org / OpenRaster wiki)
...
Krita files, on the other hand, are more complex in structure, including proprietary elements, and not designed for external programmatic access, as they are meant to be used within Krita only \parencite{KritaDocs}. % NEW: KritaDocs -- Krita documentation, .kra file format

...
With a Gaussian kernel  $G_{\sigma}$, the threshold $T(x,y)$ is computed as a Gaussian-weighted sum of local intensities subtracted by a constant $C$. Using kernel convolution ($*$), this can be expressed as \parencite{opencv_adaptive_threshold}:
$$ T(x,y) = (G_{\sigma} * I)(x,y) - C $$

In general, the weight at an offset $(i,j)$ from the kernel center can be written as follows (where $\alpha$ is a normalization constant ensuring that the whole kernel sums to 1, and $\sigma$ controls the spatial spread of the kernel) \parencite[p.~167]{DigitalImageProcessing}:
...
These embedded patches are then processed through a repeated series of normalization, feed-forward \acfp{MLP}, and in-between those, a mechanism known as self-attention that weighs the patches according to their relevance to eachother.
\begin{figure}[htp]
\centering
...
More specifically \parencite[see][]{vaswani2023attentionneed}, given the input embedding $X$, self-attention computes the projections; quieries: $Q$, keys: $K$, and values: $V$, as products of $X$ with respective learned matrices, and passes the information on as a new matrix (where $d_k$ is the dimension of $Q$ and $K$):
...
Thanks to this attention mechanism, after a number of these series-layers, the \acs{ViT} finally outputs an image embedding containing the spatial context of the image.\parencite[see][for how \acsp{ViT} work in general]{imageworth16x16words}
...
For single output masks, the mask is generated from a fourth output token \parencite[p. 17]{sam}.
...
And if no points remain after the gating processes, the mask with the highest predicted IoU score is selected as the final mask.
\begin{figure}[htp]
\centering
...
And in case the normality assumption was violated, the non-parametric domain-specific Wilcoxon signed-rank test would be used instead \parencite{Rainio2024}.
